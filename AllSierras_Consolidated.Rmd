...
sd(AllSierras_DamCapacity$volume)
...
pal1 <- hcl.colors(5,"Zissou 1")
pal2 <- hcl.colors(5,"viridis")
pal3 <- (hcl.colors(5,"plasma"))
colorspace::swatchplot(pal3, cvd = TRUE)
pal4 <- colorRampPalette(colors = c("#FDE333","#53CC67", "#009B95", "#00588B","#000000"))(5)
pal5 <- rev(hcl.colors(5,"inferno"))
pal6 <- rev(hcl.colors(5, "Blue-Red 3"))
hcl.pals(type = "sequential")
colorspace::swatchplot(hcl.colors(5,"Red-Blue"), cvd = TRUE)
...
h <- ggplot() +
  ...
  geom_sf(data = WatershedBoundaries, lwd = 0.1, fill = NA, color = "black") +
...
#Supplement Figure S1: Change in veg suitability map
```{r}
...
#Fire Risk 
#Fire Risk Continuous WHP data layer
```{r}
#Read in the raster 
...
##Supplementary Table S2: Calculate Percentages of intermittency
```{r}
#% of total stream km
...
##Supplementary Figure S2: Intermittent Dam Capacity Map
```{r}
#scales::show_col(palette_new)
...
```
##Supplementary Table S2: Total percent intermittent water storage
```{r}
...
bold_labels <-c("16050201","18040012","18040010", "18040013", "18020126","18020129", "18020125", "18040008", "18030001", "16050102")
...
map_b4 <- ggplot(data_long4, aes(x = HUCID_num, y = percentage, fill = variable)) +
  ...
  theme(axis.text.x = element_text(face = c(
    'plain', 'bold','bold','plain','plain','plain','plain','plain','bold','bold','plain','bold','bold','plain','plain','plain','plain','plain','plain','plain','plain','plain','bold','plain','bold','plain','bold','bold','plain','plain','plain')))+
  theme(legend.position = "none")
...
firerisk_normalized_Streamshq_plot <- ggplot() +
   # geom_sf(data = background, fill = "#dcdcdc", lwd = 0.1)+
    ...
    scale_fill_gradient(low = "#e8e8e8", high ="#c85a5a", name = "% of High \nRisk Areas",labels = scales::percent, breaks=c(min(selection$firerisk_normalized_Streamshq),max(selection$firerisk_normalized_Streamshq)))+
    theme_minimal()+
...
firerisk_plot <- ggplot() +
   # geom_sf(data = background, fill = "#dcdcdc", lwd = 0.1)+
    geom_sf(data = WatershedBoundaries, aes(fill = (firerisk_normalized_Streamshq)), lwd = 0.1) +
    scale_fill_gradient(low = "#e8e8e8", high ="#FFA500", name = "% Area\nWHP>2000",labels = scales::percent, breaks=c(min(selection$firerisk_normalized_Streamshq),max(selection$firerisk_normalized_Streamshq)))+
    theme_minimal()+
...
normVol_plot <- ggplot() +
  ...
  scale_fill_gradient(low = "#e8e8e8", high = "#64abce", name = "Potential Water \nStorage (m3/km)",breaks=c(round(min(selection$normVol)),round(max(selection$normVol))))+
  xlab(NULL)+
  ylab(NULL)+
...
st <- ggplot()+
  ...
  #geom_text_repel(data = WatershedBoundaries, aes(x = firerisk_normalized, y = huc_ndc, label = HUCID), size = 2)+
  theme_minimal()+
