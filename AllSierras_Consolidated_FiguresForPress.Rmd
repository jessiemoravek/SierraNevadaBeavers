---
title: "AllSierras_Consolidated_FiguresForPress"
...
##State Boundaries
```{r}
#State boundaries map for Figure 1 inset
#Downloaded from us_states package in R using 2021 census data https://docs.ropensci.org/USAboundaries/reference/us_states.html
...
background <- background %>% filter(NAME == "California" | NAME == "Nevada" | NAME == "Oregon" | NAME == "Idaho" | NAME == "Utah" | NAME == "Arizona")
...
#Palette
pal3 <- (hcl.colors(5,"plasma"))
UpMerced <- WatershedBoundaries[WatershedBoundaries$HUCID == 18040008,]

#Watershed + State boundaries
w <- ggplot() +
    geom_sf(data = background, fill = "#dcdcdc", lwd = 0.1)+
    geom_sf(data = WatershedBoundaries, fill = "#939393", lwd = 0.1, alpha = 0.5 ) +
    geom_sf(data = UpMerced, fill = "#3a3a3a", lwd = 0.1, alpha = 0.5 ) +
    ggtitle(expression(paste("Watershed Boundaries"))) +
    theme(plot.title = element_text(hjust = 0.5, size = 12),
          plot.subtitle = element_text(hjust = 0.5)) +
    theme(panel.grid.major = element_blank(),
...
ggsave("Fig_uppermerced.png", plot = w, width = 6.8, height = 8, units = "in", bg = "white", dpi=300)
...
ggsave("Fig_damcapStateBoundary.png", plot = c, width = 7, height = 4.32, units = "in", bg = "white", dpi=900)
...
writeRaster(x = riskExtract, filename = "riskExtract.tif", overwrite = TRUE)
...
##Appendix 1: Figure S3: Percent intermittent fire resilience
```{r}
##pull high quality areas
...
##use riskExtract from above

#make sure it's the right size
waterStorage <- rast("waterStorage.tif")
riskExtract_clip <- extend(riskExtract, waterStorage)

#save as raster
writeRaster(riskExtract_clip, "riskExtract_clip.tif", overwrite=TRUE)

#read in your data 
riskExtract_clip <- rast("riskExtract_clip.tif")

waterStorage[is.na(waterStorage)] <- 0

riskExtract_clip[is.na(riskExtract_clip)] <- 0
waterStorage <- mask(waterStorage, WatershedBoundaries)
riskExtract_clip <- mask(riskExtract_clip, WatershedBoundaries)
waterStorage[(waterStorage[]==0)] <- NA

r4 <- c(riskExtract_clip, waterStorage)
names(r4) <- c("riskExtract_clip","waterStorage")
```
...
# create the bivariate raster
  ...
  bivmapQ4 <- bivariate.map(rasterx = r4[["riskExtract_clip"]], rastery = r4[["waterStorage"]],
                          export.colour.matrix = TRUE,
...
```{r}
colMatrix_rasValuesriskExtract_clip %>%
...
show_col(colMatrix_rasValuesriskExtract_clip$HEX)
...
WatershedBoundaries$bivmap <- exact_extract(bivmapQ4$riskExtract_clip, WatershedBoundaries, 'frac')
WatershedBoundaries$bivmap_mode <- exact_extract(bivmapQ4$riskExtract_clip, WatershedBoundaries, 'mode')
...
getwd()
st_write(obj = ndc_cty, 
         dsn = "C:/Users/morav042/Documents/SierraNevadaBeavers/SierraNevadaBeavers", 
         layer = "WaterDeficit_County", 
...
getwd()
st_write(obj = WatershedBoundaries, 
         dsn = "C:/Users/morav042/Documents/SierraNevadaBeavers/SierraNevadaBeavers", 
         layer = "WaterDeficit_watershed", 
         driver = "ESRI Shapefile")
```
...
#Normalizing results for Figure 3
```{r}
#Notes:
...
WatershedBoundaries$npixels <- exact_extract(highRisk, WatershedBoundaries, 'count') #highRisk = WHP > 2000
WatershedBoundaries$area_m2 <- WatershedBoundaries$npixels*270*270  #multiply by area of pixels
WatershedBoundaries$area_km2 <- round(WatershedBoundaries$area_m2/1e6,2) #convert to km2
WatershedBoundaries$total_area <- as.numeric(st_area(WatershedBoundaries)) #calculate total area of watersheds
WatershedBoundaries$total_area_km2 <- round(WatershedBoundaries$total_area/1e6,2) #Watershed total area in km2
WatershedBoundaries$firerisk_normalized <- WatershedBoundaries$area_m2 / WatershedBoundaries$total_area #high risk/total watershed area



#2: firerisk_normalized_Streamshq = area of fire resilient pixels / pixels along streams 
#High fire risk pixels along streams
highRisk_Streams <- extract(highRisk, AllSierras_DamCapacity, xy = TRUE, touches = TRUE, na.rm=T) #extract high risk areas along streams
...
WatershedBoundaries$area_km2_Streams <- round(WatershedBoundaries$area_m2_Streams/1e6,2) #this is high risk fire areas along streams only (not total high risk area)

#High fire risk pixels along high quality streams (doing this to be able to normalize)
highRisk_Streamshq <- extract(highRisk, highquality, xy = TRUE, touches = TRUE) #extract high risk areas ONLY at high quality beaver streams
...
WatershedBoundaries$firerisk_normalized_Streamshq <- WatershedBoundaries$area_m2_Streamshq / WatershedBoundaries$total_area #this is high risk areas along high quality beaver streams / high risk areas along all streams
...
TableS2 <- watersheds %>%
    ...
    summarise( 
              ...
              meanHistRemain = mean(mCC_EXvHPE, na.rm=T),#remove NAs bc 0/0 = INF
              ndc = max(huc_ndc), #not actually taking the max, just need to consolidate by watershed
              fireRisk_total = max(firerisk_normalized), #not actually taking the max, just need to consolidate by watershed
              normVol = mean(VolumePerKm),
              fireResilience_streams = max(firerisk_normalized_Streamshq),#not actually taking the max, just need to consolidate by watershed
              damCapCount = sum(mCC_EX_CT), #not included in Table S2 but interesting
              totalVol = sum(volume))  #not included in Table S2 but interesting 

WatershedBoundaries <- merge(WatershedBoundaries, TableS2, by = "HUCID")
TableS2_csv <- st_drop_geometry(WatershedBoundaries)
write.csv(TableS2_csv, "TableS2_csv.csv", row.names = TRUE)
...
selection <- WatershedBoundaries %>% 
  filter(HUCID %in% c("16050201","18040012","18040010", "18040013", "18020126","18020129", "18020125", "18040008", "18030001", "16050102")) #selecting 10 highest risk watersheds (top right quadrant form previous graph)
