...
palette_biv2 <- colorRampPalette(colors = c("#574249", "#C85A5A", "#E8E8E8", "#C85A5A"))(4)
```
...
WatershedBoundaries$bivmap <- exact_extract(bivmap_simple$color, WatershedBoundaries, 'frac')
WatershedBoundaries$bivmap_mode <- exact_extract(bivmap_simple$color, WatershedBoundaries, 'mode')
WatershedBoundaries$bivmap_mode <- as.factor(WatershedBoundaries$bivmap_mode)
#plot to look -- only have 2 (fire) and 3 (neither)
WatershedBoundaries$bivmap_mode %>% as.factor() %>% 
  mutate(both = 1, fire = 2, neither = 3, water = 4)

ggplot() +
  geom_sf(data = WatershedBoundaries, aes(fill = bivmap_mode)) +
