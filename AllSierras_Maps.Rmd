...
#make sure it's the right size
fireExtract <- extend(fireExtract, contFireRisk)

#save as raster
writeRaster(fireExtract, "fireExtract.tif", overwrite=T)

#read in your data 
fireExtract <- rast("fireExtract.tif")
waterStorage <- rast("waterStorage.tif")
waterStorage[(waterStorage[] == 0)] <- NA
fireExtract[(fireExtract[] == 0)] <- NA
r4 <- c(fireExtract, waterStorage)
names(r4) <- c("fireExtract","waterStorage")
```
...
# create the bivariate raster
#if (!file.exists("bivmapQ.tif")) {
  bivmapQ4 <- bivariate.map(rasterx = r4[["waterStorage"]], rastery = r4[["fireExtract"]],
                          export.colour.matrix = TRUE,
...
# Make the map using ggplot
map_q4 <- ggplot() +
    geom_raster(data = bivMapDFQ4, aes(x = x, y = y, fill = bivVal)) +
...
map_q4 <- map_q4 + inset_element(BivLegend, 0, 0, 0.2, 0.3)
...
#Bivariate cloropleth NONRASTER
##Data wrangling
```{r}
...
test <- merge(watersheds, df, by.x = 0, by.y =  "ID")

watersheds <- st_as_sf(watersheds)
WatershedBoundaries <- st_as_sf(WatershedBoundaries)
watersheds$HUCID == WatershedBoundaries$HUCID
test <- merge(WatershedBoundaries, watersheds, by = "HUCID")
```
...
ggplot()+
  geom_point(data = WatershedBoundaries, aes(x = firerisk_normalized, y = (huc_ndc)))+
  ...
  ggtitle(expression(paste("Water Deficit vs. NDC by watershed"))) +
  theme(plot.title = element_text(hjust = 0.5, size = 12),
          plot.subtitle = element_text(hjust = 0.5)) +
    #theme(element_text(size=10),legend.position = c(.15,.2))+
