...
```{r}
# Read in the shapefile with st_read()
beavers <- st_read("C:/Users/jessie_moravek/Desktop/SierraNevadaBeavers/inatbeavers.shp")

#remove unneeded shapefiles
file.remove(list.files(pattern = "inatbeavers*",recursive = F))

crs(beavers) ==crs(AllSierras_DamCapacity)

#Project to wkt (NASA projection)
beavers <- st_transform(beavers, 3310)
...
background_crop <- st_crop(background, beavers)
background_crop <- st_crop(background, xmin = -124.5, xmax = -114,
                                    ymin = 31.5, ymax = 42.5)
...
beav <- ggplot() +
    geom_sf(data = background_crop, fill = "#dcdcdc", lwd = 0.1)+
    #geom_sf(data = WatershedBoundaries, fill = "#b0b0b0", lwd = 0.1, alpha = 0.5 ) +
    geom_sf(data = beavers, color = "black",size = .5)+
    scale_color_manual(values=palette_new)+
    theme_minimal()+
    ggtitle(expression(paste("Watershed Boundaries"))) +
    theme(plot.title = element_text(hjust = 0.5, size = 12),
...
ggsave("Fig_beavs.png", plot = beav, width = 6.8, height = 8, units = "in", bg = "white", dpi=300)
...
c <- ggplot() +
    ...
    theme_minimal()+
    ggtitle(expression(paste("b) Current Dam Capacity"))) +
    theme(plot.title = element_text(hjust = 0.5, size = 12),
          plot.subtitle = element_text(hjust = 0.5)) +
...
h <- ggplot() +
  ...
  theme_minimal() +
  ggtitle(expression(paste("a) Historical Dam Capacity"))) +
  theme(
...
r <- ggplot() +
    ...
    scale_color_manual(values=palette_new,
                        labels = c("0", "1-2", "2-5", "5-15", "15-40"))+
    theme_minimal()+
    ggtitle(expression(paste("c) Percent Capacity Remaining"))) +
    theme(plot.title = element_text(hjust = 0.5, size = 12),
          plot.subtitle = element_text(hjust = 0.5)) +
    theme(legend.text = element_text(size=8),
          legend.title = element_text(size = 8),
...
ggsave("Fig_damcaphist.png", plot = fig2, width = 7, height = 4.32, units = "in", bg = "white", dpi=900)
```
...
png(filename = "Fig_scatterplot.png", width = 8, height =5, units = "in", res = 800) 

scatter <- ggplot()+
  geom_point(data = WatershedBoundaries, aes(x = firerisk_normalized, y = (huc_ndc.x)))+ 
  geom_vline(xintercept = x_value, color = "black", linetype = "solid") +
  geom_hline(yintercept = y_value, color = "black", linetype = "solid") +
  #geom_text(data = WatershedBoundaries, aes(x = firerisk_normalized, y = huc_ndc, label = HUCID), vjust = -1.5, size = 2) +
  #geom_text_repel(data = WatershedBoundaries, aes(x = firerisk_normalized, y = huc_ndc.x, label = HUCID), size = 2)+
  ...
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())+
  theme(axis.title.x = element_text(size = 14),  # Adjust the size of x-axis labels
...
ggsave("Fig_scatterplot.png", plot = scatter, width = 8, height = 5, units = "in", bg = "white", dpi=300)

```
##Risk selection for presentation
```{r}
...
selection_highpriority <- ggplot() +
   # geom_sf(data = background, fill = "#dcdcdc", lwd = 0.1)+
    geom_sf(data = WatershedBoundaries, fill = NA, lwd = 0.1) +
    geom_sf(data = selection, fill = "#6c83b5", lwd = .6) +
...
ggsave("Fig_selection_highpriority.png", plot = selection_highpriority, width = 3, height = 5.3, units = "in", bg = "white", dpi=300)

```
##high potential for presentation
```{r}
selection1 <- WatershedBoundaries %>% 
  filter(HUCID %in% c("18040011","18020125","18040012", "18020126", "18020128","18020129", "18040013", '18010122'))

selection_highpotential <- ggplot() +
   # geom_sf(data = background, fill = "#dcdcdc", lwd = 0.1)+
    geom_sf(data = WatershedBoundaries, fill = NA, lwd = 0.1) +
    geom_sf(data = selection1, fill = "#73ae80", lwd = .6) +
    theme_minimal()+
    #ggtitle(expression(paste("Potential Water Storage"))) +
    theme(plot.title = element_text(hjust = 0.5, size = 12),
          plot.subtitle = element_text(hjust = 0.5),
...
ggsave("Fig_selection_highpotential.png", plot = selection_highpotential, width = 3, height = 5.3, units = "in", bg = "white", dpi=300)
```
...
selection2 <- WatershedBoundaries %>% 
  filter(HUCID %in% c("18040012","18040013","18020126","18020129","18020125"))

All3 <- ggplot() +
   # geom_sf(data = background, fill = "#dcdcdc", lwd = 0.1)+
    geom_sf(data = WatershedBoundaries, fill = "#e8e8e8", lwd = 0.1) +
    geom_sf(data = selection, fill = "#6c83b5", lwd = .1)+
    geom_sf(data = selection1, fill = "#73ae80", lwd = .1) +
    geom_sf(data = selection2, fill = "#2a5a5b", lwd = .1) +
    theme_minimal()+
    #ggtitle(expression(paste("Potential Water Storage"))) +
    theme(plot.title = element_text(hjust = 0.5, size = 12),
          plot.subtitle = element_text(hjust = 0.5),
...
ggsave("Fig_all3.png", plot = All3, width = 3, height = 5.3, units = "in", bg = "white", dpi=300)
```
