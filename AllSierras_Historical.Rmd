...
```{r}
WatershedBoundaries <- mutate(WatershedBoundaries, ID=row_number())
watersheds <- st_join(AllSierras_DamCapacity,WatershedBoundaries)
?st_join
...
ggplot() +
    geom_sf(data = watersheds, aes(x = totalVol)) +
    scale_color_viridis(discrete = FALSE, option = "plasma", na.value = "grey70")+
    xlab(NULL)+
    ylab(NULL)+
    ggtitle(expression(paste("Change in Volume (m3)"))) + 
...
ggplot() + 
  geom_sf(data = test, fill = test$totalVol) +
  scale_color_viridis(discrete = FALSE, option = "plasma", na.value = "grey70")+
  xlab(NULL)+
  ylab(NULL)+
  ggtitle(expression(paste("Change in Volume (m3)"))) + 
