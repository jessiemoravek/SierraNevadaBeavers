...
damCapacity2_extract <- damCapacity2_centroid %>%
  ...
  select(lon, lat, oCC_EX) %>%
  st_drop_geometry()

#Check what it looks like
ggplot()+
  geom_sf(data = damCapacity2_extract) #Thumbs up looks good 

#save as .txt
write.table(damCapacity2_extract,"damCapacity_centroid.txt",sep="\t",row.names=FALSE, col.names=TRUE)
