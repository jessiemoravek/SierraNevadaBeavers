...
study_area5 = st_sf(geom=study_area4)
class(study_area5)
...
```{r}
ggplot() +
   geom_sf(data = study_area4, fill = NA) +
    #geom_sf(data = damCapacity2, col = "blue") +
    xlab(NULL)+
    ylab(NULL)+
    ggtitle( expression(paste("Upper Merced 1km Grid"))) + 
...
##Volume by grid cell
```{r}
intersection4 <- st_intersection(study_area5, damCapacity2_v) %>% 
  mutate(length = st_length(.)) %>% #creates a "length" column that is the length of that particular segment in the grid cell from the "id" column
  st_drop_geometry() # complicates things in joins later on

study_area5 <- study_area5 %>% 
  left_join(intersection4, by = "ID")

vol_by_sa5 <- study_area5 %>% 
  mutate(percent_len_in_cell = length/iGeo_Len) %>% 
  mutate(segment_vol_in_cell = percent_len_in_cell*volume) %>% 
  group_by(ID) %>% summarize(total_vol_in_cell = sum(segment_vol_in_cell))

vol_by_sa5$total_vol_in_cell <- as.numeric(vol_by_sa5$total_vol_in_cell) #convert volume col to numeric
saveRDS(vol_by_sa5,"vol_by_sa5.rds")
vol_by_sa5 <- readRDS("vol_by_sa5.rds")
sum(vol_by_sa5$total_vol_in_cell, na.rm=T)
...
study_area <- as(extent, "SpatialPolygons")
```
...
proj4string(study_area) <- CRS(wkt)
```

```{r}
# Create a raster grid with 1 km resolution
grid_res <- 0.01 # 1 km in meters
e <- extent(study_)
grid_raster <- raster::raster(study_area3)
res(grid_raster) <- 1000
#not working for some reason
```

```{r}
# Calculate the grid resolution in degrees
grid_res <- 0.01  # 0.01 degrees is approximately 1 km

# Calculate the number of cells in x and y direction
n_cells_x <- ceiling((xmax - xmin) / grid_res)
n_cells_y <- ceiling((ymax - ymin) / grid_res)

# Create a regular grid over the study area
grid <- GridTopology(
  cellcentre.offset = c(xmin + grid_res / 2, ymin + grid_res / 2),
  cellsize = c(grid_res, grid_res),
  cells.dim = c(n_cells_x, n_cells_y)
)
```

```{r}
# Convert the grid to a spatial grid object
grid_sp <- SpatialGrid(grid, proj4string = CRS(proj4string(study_area)))

```

```{r}
# Convert the spatial grid to a spatial points object
grid_points <- as(grid_sp, "SpatialPoints")
```


```{r}
# Project the points back to the geographic coordinate system
grid_points_geo <- spTransform(grid_points, CRS(wkt))
```

```{r}
#Check 
n_cells_x <- grid_points_geo@cells.dim[1]
n_cells_y <- grid_points_geo@cells.dim[2]

plot(grid_points_geo)
```
...
boundingBox2 <- st_transform(boundingBox2, crs = wkt)
...
boundingBox_tealalbers <- st_transform(boundingBox2, wkt)
#Check if sf object is in metric CRS (i.e. projected)
st_is_longlat(boundingBox2)
#st_is_longlat returns "False" which means that the object is in a metric CRS
...
grid3 <- st_transform(grid3, wkt)
dim(grid3)


#using a raster 

gridr <- raster(boundingBox2, nrows = 63, ncols = 174,crs = wkt)            
dim(gridr)
?rasterize
