...
for (i in damCapClip_v){
  damDens <- damCapClip_v$mCC_EX_CT
...
for (i in damCapacity_v){
  damDens <- damCapacity_v$mCC_EX_CT
...
for (i in damCapacity2_v){
  damDens <- damCapacity2_v$mCC_EX_CT
  slope <- damCapacity2_v$iGeo_Slope #meters
  da <- damCapacity2_v$iGeo_DA #m2
  width <- ifelse(da < 4.95,2.18*(da)^0.191,
                  ifelse(da >= 4.95 & da < 377,1.41*(da)^0.462,7.18*(da)^0.183))
  length <- 1/slope
  maxLength<- (damCapacity2_v$iGeo_Len/damCapacity2_v$mCC_EX_CT)
    volume <- ifelse(length < maxLength, 0.5*1*(width)*(length)*(damDens),1*(width)*(maxLength)*(damDens))
  surfaceArea <- ifelse(length < maxLength, (width)*(length)*(damDens),(width)*(maxLength)*(damDens))
...
saveRDS(sa_by_grid3,"sa_by_grid3")
```
###Map
```{r}
ggplot() +
